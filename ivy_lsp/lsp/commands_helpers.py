"""Helper functions for Ivy LSP command handlers."""
...
@dataclass
class _ServerProxy:
    """Lightweight proxy exposing only the indexer for visualization handlers."""
...
def _track_start(srv: Any, op_type: str, filepath: str) -> Optional[str]:
    """Record the start of an Ivy CLI operation via the state tracker."""
...
def _track_end(srv: Any, op_id: Optional[str], result: Dict[str, Any]) -> None:
    """Record the successful end of an Ivy CLI operation."""
    if op_id is None:
        return
    tracker = getattr(srv, "state_tracker", None)
    if tracker is None:
...
def _track_error(srv: Any, op_id: Optional[str], exc: Exception) -> None:
    """Record a failed Ivy CLI operation."""
    if op_id is None:
        return
    tracker = getattr(srv, "state_tracker", None)
    if tracker is None:
...
def _refresh_open_diagnostics_sync(srv: Any) -> None:
    ...
    for uri, doc in items:
        ...
        try:
            ...
            logger.warning("Failed to refresh diagnostics for %s", uri, exc_info=True)


async def _refresh_open_diagnostics_async(srv: Any) -> None:
