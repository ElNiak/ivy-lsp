...
class TestNctClassification:
    ...
    def test_require_before_is_assumption(self):
        assert (
            classify_requirement(_req("require", "before"))
            == NctClassification.ASSUMPTION
        )

    def test_require_after_is_guarantee(self):
        assert (
            classify_requirement(_req("require", "after"))
            == NctClassification.GUARANTEE
        )

    def test_ensure_after_is_guarantee(self):
        assert (
            classify_requirement(_req("ensure", "after")) == NctClassification.GUARANTEE
        )

    def test_assert_after_is_guarantee(self):
        assert (
            classify_requirement(_req("assert", "after")) == NctClassification.GUARANTEE
        )

    def test_assume_before_is_assumption(self):
        assert (
            classify_requirement(_req("assume", "before"))
            == NctClassification.ASSUMPTION
        )
    ...
    def test_require_direct_is_assumption(self):
        assert (
            classify_requirement(_req("require", "direct"))
            == NctClassification.ASSUMPTION
        )

    def test_ensure_direct_is_guarantee(self):
        assert (
            classify_requirement(_req("ensure", "direct"))
            == NctClassification.GUARANTEE
        )
    ...
    def test_require_around_is_guarantee(self):
        ...
        assert (
            classify_requirement(_req("require", "around"))
            == NctClassification.GUARANTEE
        )

    def test_ensure_around_is_guarantee(self):
        assert (
            classify_requirement(_req("ensure", "around"))
            == NctClassification.GUARANTEE
        )

    def test_assume_around_is_guarantee(self):
        assert (
            classify_requirement(_req("assume", "around"))
            == NctClassification.GUARANTEE
        )
...
class TestActionClassification:
    ...
    def test_exported_is_generated(self):
        ...
        assert (
            classify_action_direction("quic.send", scope)
            == ActionClassification.GENERATED
        )
    ...
    def test_imported_is_received(self):
        ...
        assert (
            classify_action_direction("tls.handshake", scope)
            == ActionClassification.RECEIVED
        )
    ...
    def test_neither_is_internal(self):
        ...
        assert (
            classify_action_direction("helper.compute", scope)
